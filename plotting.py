...
def plot_cell_compartments(cell):
    for comp_idx in xrange(len(cell.xmid)):
        pl.plot(cell.xmid[comp_idx], cell.zmid[comp_idx],\
...
def simple_plot_2D(cell, plot_range, clamp_1, clamp_2):
    ...
    def init():
        scat = ax.scatter(x,z, c=vmem[:,0], s=10*comp_size, vmin=-60, vmax=20)
        stim_point, = ax.plot([cell.xmid[stim_idx_1]], [cell.zmid[stim_idx_1]], \
                              'D', color='w')
        ax.plot([cell.xmid[stim_idx_2]], [cell.zmid[stim_idx_2]], 'D', color='y')
        ax.plot([cell.xmid[syn_numb]], [cell.zmid[syn_numb]], 'D', color='pink')
    ...
    ax4.set_title('vmem at inj. point %d (b)' %syn_numb)
    ...
    ani = animation.FuncAnimation(fig, update_plot, frames=xrange(n_tsteps),
                                  fargs=(vmem, scat, time_text, time_bar, stim_point), blit=True, interval=.1, init_func=init)
