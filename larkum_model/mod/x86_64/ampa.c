	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  hoc_register_dparam_size(_mechtype, 3);
 	...
 	ivoc_help("help ?1 ampa /home/torbjone/bedlewo/project/larkum_model/mod/x86_64/ampa.mod\n");
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
 ...
 {int  _lcount ;for ( _lcount = 0 ; _lcount <= ((int) Nspike ) - 1 ; _lcount ++ ) {
     if ( at_time ( nrn_threads, ((double) _lcount ) * Tspike + del ) ) {
       state_discontinuity ( _cvode_ieq + 0, & gampa , gampa + gmax ) ;
       }
     } }
...
static _initlists() {
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = &(gampa) - _p;  _dlist1[0] = &(Dgampa) - _p;
_first = 0;
}
